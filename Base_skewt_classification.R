...
for(i in 1:12){
  ...
  xbar.snow[,i] = apply(snow.train, 2, mean)
...
#     print("snow k:");print(k)
#     }
#   pm.snow=pm
#   colnames(pm.snow)=c('xi','omega','alpha','nu')
#   
  ##############################
  ############ RAIN ############
  ##############################
  ...
  for(k in 1:16){
    ...
    pm[k,]=st.i@param$dp
    #lines(x.seq,dst(x.seq,dp=pm[k,]),col=4,lwd=2)
    print("rain k:");print(k)
  }
  pm.rain=pm
  colnames(pm.rain)=c('xi','omega','alpha','nu')
...
#   
  ...
  pm.fzra=pm
  colnames(pm.fzra)=c('xi','omega','alpha','nu')
...
BS.ref = 0.218555#sum(BS.ref.test, na.rm = T)/sum(n.test) #0.218555
