...
####################### SNOW #######################
  ...
  ####################### RAIN #######################
  prb.rain= as.data.frame(cbind(utms,pi.rain))
  ...
  #### Kriging Using GSTAT package ####
  initl = vgm(psill=0.00008,model="Exp",range=20)
  ...
  #### Kriging Using GSTAT package ####
  initl = vgm(psill=0.0018,model=mod.fzra.k[i],range=20)
  ...
  pdf(file = paste("figures/Priors/Kriging_preds", m,".pdf", sep="_"))
  par(mfrow=c(2,2))
...
quilt.plot(lon.new,lat,tps.pred.ip[,i],main="Pellets TPS Predictions",zlim=c(0,max(tps.pred.ip[,i])))
map("world", add = T)
map("state", add = T)

quilt.plot(lon.new,lat,tps.pred.fzra[,i],main="Freezing TPS Predictions",zlim=c(0,max(tps.pred.fzra[,i])))
map("world", add = T)
map("state", add = T)
#dev.off()
