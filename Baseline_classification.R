...
pi.snow=read.table(file="Predicted_Values/LOOCV_snow.txt")
pi.rain=read.table(file="Predicted_Values/LOOCV_rain.txt")
pi.ip=read.table(file="Predicted_Values/LOOCV_ip.txt")
pi.fzra=read.table(file="Predicted_Values/LOOCV_fzra.txt")

# pi.snow=read.table(file="Predicted_Values/TPS_snow.txt")
# pi.rain=read.table(file="Predicted_Values/TPS_rain.txt")
# pi.ip=read.table(file="Predicted_Values/TPS_ip.txt")
# pi.fzra=read.table(file="Predicted_Values/TPS_fzra.txt")
...
ab.BSS = function(param){
  ...
  for(j in 1:n.train[i]){
    ...
    phi.snow = dmvnorm(train.temp[j,], mean = xbar.snow[,i], sigma = sigma.reg.snow[[i]])
    phi.rain = dmvnorm(train.temp[j,], mean = xbar.rain[,i], sigma = sigma.reg.rain[[i]])
    phi.ip = dmvnorm(train.temp[j,], mean = xbar.ip[,i], sigma = sigma.reg.ip[[i]])
...
#   pow=apply(train.temp,2,powerTransform)
#   for(j in 1:16){
#     powers[j,i]=pow[[j]]$lambda
#   }
#   
#   hist(train.temp[,i])
#   hist(train.temp[,i], breaks ="FD",xlab="Temperature", main= paste("Level 1 Training set ",i),prob=T, ylim=c(0,0.08))#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(train.temp[,i]), sd=sd(train.temp[,i])), col="blue", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(train.temp[,i]),col="blue", lwd=3)
#   
#   train.temp.norm = bcPower(train.temp,lambda=powers[,i])
#   hist(train.temp.norm[,i])
#   hist(train.temp.norm[,i], breaks ="FD",xlab="Temperature", main= paste("Level 1 Normal Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(train.temp.norm[,i]), sd=sd(train.temp.norm[,i])), col="blue", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(train.temp.norm[,i]),col="blue", lwd=3)
#   
...
#   rain.train.norm = bcPower(rain.train,lambda=powers.rain[,i])
#   rain.train.norm2 = matrix(nrow=length(rain.train.norm[,1]),ncol=16)
#   for(j in 1:16){
#   rain.train.norm2[,j]=reflect.rev(rain.train.norm[,j], powers.rain[j,i])
# }
  #################################
  ############ PELLETS ############
  #################################
...
#   fzra.train2=apply(fzra.train,2,reflect)
#   pow.fzra=apply(fzra.train,2,powerTransform)
#   for(j in 1:16){
#     powers.fzra[j,i]=pow.fzra[[j]]$lambda
#   }
#   
#   #powers.fzra[,i]=array(0,dim=16)
#   powers.fzra[,i]
#   fzra.train.norm = bcPower(fzra.train,lambda=powers.fzra[,i])
#   
...
#   par(mfrow=c(1,4))
#   hist(snow.train[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Snow Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(snow.train[,1]), sd=sd(snow.train[,1])), col="blue", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(snow.train[,1]),col="blue", lwd=3)
#   
#   hist(rain.train[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Rain Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(rain.train[,1]), sd=sd(rain.train[,1])), col="darkgreen", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(rain.train[,1]),col="darkgreen", lwd=3)
#   
#   hist(ip.train[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Pellets Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(ip.train[,1]), sd=sd(ip.train[,1])), col="purple", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(ip.train[,1]),col="purple", lwd=3)
#   
#   hist(fzra.train2[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Freeze Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(fzra.train2[,1]), sd=sd(fzra.train2[,1])), col="red", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(fzra.train2[,1]),col="red", lwd=3)
#   
# #   hist(fzra.train[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Freeze Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
# #   plot(function(x) dt(x, df=length(fzra.train[,1])), col="red", lwd=2, add=TRUE, yaxt="n")
# #   abline(v=mean(fzra.train[,1]),col="red", lwd=3)
# #   
#   
#   
#   par(new=F)
# par(mfrow=c(1,4))
#   hist(snow.train.norm[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Snow Normal Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(snow.train.norm[,1]), sd=sd(snow.train.norm[,1])), col="blue", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(snow.train.norm[,1]),col="blue", lwd=3)
#   
#   hist(rain.train.norm2[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Rain Normal Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(rain.train.norm2[,1]), sd=sd(rain.train.norm2[,1])), col="darkgreen", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(rain.train.norm2[,1]),col="darkgreen", lwd=3)
#   
#   hist(ip.train.norm[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Pellets Normal Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(ip.train.norm[,1]), sd=sd(ip.train.norm[,1])), col="purple", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(ip.train.norm[,1]),col="purple", lwd=3)
#   
#   hist(fzra.train.norm[,1], breaks ="FD",xlab="Temperature", main= paste("Level 1 Freeze Normal Training set ",i),prob=T)#;axis(4,at=mean.rain,label=round(mean.rain,4))
#   curve(dnorm(x, mean=mean(fzra.train.norm[,1]), sd=sd(fzra.train.norm[,1])), col="red", lwd=2, add=TRUE, yaxt="n")
#   abline(v=mean(fzra.train.norm[,1]),col="red", lwd=3)
#   
  ...
  #for(k in 1:16){
  test.snow = t(apply(test.temp,1,function(x) x^t(powers.snow[,i])))
  test.ip = t(apply(test.temp,1,function(x) x^t(powers.ip[,i])))
  #}
  ...
  #    a = ab.all[i, 1]
  #    b = ab.all[i, 2]
  #    sigma.reg.snow[[i]] = a[i]*sigma.snow[[i]] + b[i]*diag(1,16,16)
  #    sigma.reg.rain[[i]] = a[i]*sigma.rain[[i]] + b[i]*diag(1,16,16)
  #    sigma.reg.ip[[i]] = a[i]*sigma.ip[[i]] + b[i]*diag(1,16,16)
  #    sigma.reg.fzra[[i]] = a[i]*sigma.fzra[[i]] + b[i]*diag(1,16,16)
...
#     phi.fzra = dmvnorm(test.temp[j,], mean = xbar.fzra[,i], sigma = sigma.fzra[[i]])
    ...
    phi.snow = dmvnorm(test.snow[j,], mean = xbar.snow[,i], sigma = sigma.snow[[i]])
    phi.rain = dmvnorm(test.temp[j,], mean = xbar.rain[,i], sigma = sigma.rain[[i]])
    phi.ip = dmvnorm(test.ip[j,], mean = xbar.ip[,i], sigma = sigma.ip[[i]])
    phi.fzra = dmvnorm(test.temp[j,], mean = xbar.fzra[,i], sigma = sigma.fzra[[i]])
...
BS.ref =.218555#sum(BS.ref.test, na.rm = T)/sum(n.test) #0.218555
BS = sum(BS.test, na.rm = T)/sum(n.test)#0.0854115
BSS = 1 - (BS/BS.ref) #0.6091995

accuracy = sum(correct.class.tot, na.rm = T)/sum(n.test) #Baseline 0.8640162 #LOOCV 0.8417122  #TPS  0.8516091
